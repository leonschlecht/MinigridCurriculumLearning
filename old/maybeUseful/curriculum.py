...
allEnvs = ["MiniGrid-DoorKey-5x5", "MiniGrid-DoorKey-6x6", "MiniGrid-DoorKey-8x8", "MiniGrid-DoorKey-16x16"]


# env and name are full env and full model name
def train(env, model):
    global logger
    os.system("python3 -m scripts.train --algo ppo --env " + env + " --frames=" + str(
        frames) + " --model " + model + " --save-interval 10000")
    logger.info('Trained ' + env + ' using ppo to folder ' + model + ' with ' + str(0) + ' frames')
    enjoy_cmds.append('python3 -m scripts.enjoy --env ' + env + ' --model ' + model)
    rewards = 0
    for testEnv in allEnvs:
        os.system("python3 -m scripts.evaluate --env " + testEnv + " --model " + model)
        print("path = " + 'storage/' + model + '/evaluation.txt')
        with open('storage/' + model + '/evaluation.txt', 'r') as f:
            text = f.readlines()
            print(text)
